const TV_ID = location.pathname.split('/')[3];
...
var setBookMark = function () {
    const url = '/api/book-mark/tv/set.api';
    ...
    API_CALL.post(url, param, function (result, message, data) {
        ...
        if (data.bookMarkYn) {
            $('#tv-bookmark').css('color', 'coral');
        } else {
            $('#tv-bookmark').css('color', 'black');
        }
    })
}
...
var setComment = function (commentId, id) {
    const url = '/api/comment/tv/set.api';
...
var getCommentList = function (pageIndex) {
    const url = '/api/comment/tv/gets.api';
...
var getReCommentList = function (commentId) {
    const url = '/api/comment/tv/re/gets.api';
...
var getSetComment = function (id) {
    const url = '/api/comment/tv/set/get.api';
...
var changeInput = function (comment) {
    const $div = $('#tv_comment_' + comment.id);
    $div.empty();
...
var drawingCommentList = function (list, totalCount, pageIndex) {
    ...
    list.forEach(function (comment) {
        ...
        html += '   <div style="margin: 5px 0px" id="tv_comment_' + comment.id + '">' + comment.contents.replaceAll("\n", "<br/>") + '</div>';
